...
- **📦 Comprehensive Cookie Store Tab (Tab #4)**: Master-detail inventory storing all request and response cookies, tracking exact cookie value repetition counts, Set-Cookie attributes (`HttpOnly`, `Secure`, `SameSite`), domain associations, proxy history ingestion, and native Montoya editor deep-linking with cookie highlighting.
...
- **Burp Suite Interoperability**: Context menu ingestion (`⏱️ Send to Session Expiration Checker`, `🍪 Send to Session Cookie Finder`, `📦 Send to Cookie Store`, `📦 Open Cookie Store`) and dispatch to Repeater, Intruder, and Organizer.
