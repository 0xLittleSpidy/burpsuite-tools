 ...
 *
 * Follows the Burp Suite extension UI standards from extension_architecture.md.
 *
 * @author littlespidy
 */
public class MultiSelectFilterButton extends JButton {
    ...
    /**
     * @param label     Displayed on the button face, e.g. "Status \u25be"
     * @param options   The selectable items (first item is treated as "All / clear" sentinel if it starts with "All")
     ...
     * The filter should pass all entries when this returns true.
     */
    public boolean isAllSelected() {
        return getSelected().isEmpty();
    }

    /** Programmatically reset to "all cleared" state. */
    ...
    /** Programmatically set selected items. */
    public void setSelected(Set<String> items) {
        for (Map.Entry<String, JCheckBox> entry : checkBoxMap.entrySet()) {
            if (!isAllSentinel(entry.getKey())) {
                entry.getValue().setSelected(items != null && items.contains(entry.getKey()));
            }
        }
        JCheckBox allCb = checkBoxMap.get(options.isEmpty() ? "" : options.get(0));
    ...
    private void showPopup() {
        ...
        scroll.setPreferredSize(new Dimension(240, Math.min(options.size() * 28 + 16, 300)));
