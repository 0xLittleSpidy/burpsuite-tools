...
/**
 * KeyHacks Credential Verification Engine ported for Response Inspector.
 *
...
public class SecretVerifierService {
    ...
    private static void loadTemplates() {
        try (InputStream is = SecretVerifierService.class.getResourceAsStream("/com/littlespidy/responseinspector/keyhacks_verifications.tsv")) {
            if (is != null) {
    ...
    /**
     * Generates a complete verification specification for the given finding entry.
     */
    public static SecretVerificationSpec generateVerification(FindingEntry finding) {
        if (finding == null) return getUniversalFallback("No secret provided", "");

        String rawToken = finding.matchValue() != null ? finding.matchValue().strip() : "";
        String ruleName = finding.patternName() != null ? finding.patternName() : "";
        String category = "Secrets";
        ...
        if (rawToken.startsWith("AKIA") || rawToken.startsWith("ASIA") || ruleName.toLowerCase().contains("aws access key")) {
            return new SecretVerificationSpec(
                ...
                "<GetCallerIdentityResult> with Account and Arn elements.",
                "<Error><Code>InvalidClientTokenId</Code> or <Code>SignatureDoesNotMatch</Code>",
                "Non-destructive identity query. Does not modify IAM policies or state.",
                "https://docs.aws.amazon.com/STS/latest/APIReference/API_GetCallerIdentity.html"
            );
        }

        // 2. Direct Template Match
    ...
    private static TemplateEntry findMatchingTemplate(String ruleName, String category, String token) {
        ...
        String ruleLower = ruleName.toLowerCase(Locale.ROOT);

        // Priority direct keyword matching
        for (TemplateEntry t : TEMPLATES) {
            for (String kw : t.keywords()) {
                if (!kw.isEmpty() && ruleLower.contains(kw.toLowerCase(Locale.ROOT))) {
                    return t;
                }
            }
        }
    ...
    private static String injectToken(String template, String token) {
        ...
        // Common KeyHacks placeholder patterns
        String[] placeholders = {
     ...
     */
    public static VerificationRequestDetails getVerificationRequestDetails(FindingEntry finding) {
        if (finding == null) {
        ...
        String ruleName = finding.patternName() != null ? finding.patternName() : "";
        String category = "Secrets";
        String sourceUrl = finding.url() != null ? finding.url() : "";
        ...
        if (matchedTemplate != null) {
            ...
            if (!headers.containsKey("User-Agent")) {
                headers.put("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) BurpResponseInspector/1.0");
            }
    ...
    private static VerificationRequestDetails buildFallbackDetails(String serviceName, String host, int port, boolean secure, String token) {
        ...
        String raw = "GET /user HTTP/1.1\r\n" +
            "Host: " + cleanHost + "\r\n" +
            "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) BurpResponseInspector/1.0\r\n" +
     ...
     */
    public static HttpRequest buildVerificationRequest(FindingEntry finding) {
        VerificationRequestDetails details = getVerificationRequestDetails(finding);
