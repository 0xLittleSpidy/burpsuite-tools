...
- **📖 Integrated Cookie Documentation & Classifier (`cookiesearch.org`)**: Embedded documentation tab modeled after `HeaderDocPanel` in Header Inspector, backed by an offline pre-bundled knowledge base of 706 scraped cookie definitions (`cookiesearch.org`), real-time category badges, non-blocking background web crawler for novel tokens, and security heuristics fallback.
