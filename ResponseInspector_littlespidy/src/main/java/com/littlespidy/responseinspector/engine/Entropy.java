...
/**
 * Shannon-entropy and false-positive suppression engine ported from Ghost-Js and JS SourceMap Explorer.
 * Suppresses documented sample credentials, public-by-design keys, placeholder/template
 * text, UI/natural-language strings, generic repetitive sequences, and low-entropy tokens.
 *
 * @author littlespidy
 */
public final class Entropy {
    ...
    public static boolean isSuppressed(String value) {
        return isLikelyFalsePositive(value, "Generic", false);
    }

    // ── Documented sample credentials from tutorials/READMEs ─────────────────
    ...
    private static final String[] KNOWN_EXAMPLE_FRAGMENTS = {
        "EXAMPLEKEY", "AKIAIOSFODNN7EXAMPLE", "your-api-key", "your_api_key", "change_me", "changeme"
    };
    ...
    private static final Pattern[] COMMON_FP = {
        ...
        Pattern.compile("^(abc|123|test|demo|example|sample|default|changeme|password|secret|admin)", Pattern.CASE_INSENSITIVE),
        Pattern.compile("^(undefined|null|none|empty|todo|fixme|placeholder|insert|your)", Pattern.CASE_INSENSITIVE),
    ...
    // Names whose real format legitimately IS 32 hex chars — exempt from hex32 FP rule
    ...
    /**
     * Determines whether a candidate secret value is likely a false positive.
     *
     * @param value the raw matched secret value
     * @param secretName the rule / pattern name (used for format exemptions)
     * @param needsEntropy whether extra Shannon entropy thresholding should be applied
     * @return true if the value is deemed a false positive and should be suppressed
     */
    public static boolean isLikelyFalsePositive(String value, String secretName, boolean needsEntropy) {
        ...
        // 1. Documented sample credentials
        if (KNOWN_EXAMPLE_VALUES.contains(v)) return true;
        for (String frag : KNOWN_EXAMPLE_FRAGMENTS) {
            if (v.contains(frag)) return true;
        }

        // 2. Public-by-design keys (Klaviyo private key uses pk_ but is real)
        if (!"Klaviyo Private Key".equals(secretName)) {
            for (Pattern p : PUBLIC_BY_DESIGN) {
                if (p.matcher(v).find()) return true;
            }
        }

        // 3. Generic FP shapes, with hex32 exemption
    ...
    private static boolean isUiOrNaturalLanguage(String v) {
        ...
        int common = 0;
        for (String w : words) {
            if (COMMON_WORDS.contains(w.toLowerCase())) common++;
        }
